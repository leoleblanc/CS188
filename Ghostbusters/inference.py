...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        if self:
            ...
            if sumVals == 0:
                return
            for key in self:
                self[key] = float(self[key]) / sumVals
    ...
    def sample(self):
        ...
        for key in self:
            weightList.append(float(self[key]) / sumVals)
            if value < sum(weightList):
                return key
...
class ExactInference(InferenceModule):
    ...
    def predict(self, gameState):
        ...
        #the point of this one is to add up all positionDistributions for each position    
        for old in self.allPositions:
            oldProb = self.beliefs[old]
            newPosDist = self.getPositionDistribution(gameState, old)
            for new in newPosDist:
                probabilities[new] += newPosDist[new] * oldProb
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for particle in range(self.numParticles):
            self.particles.append(legalPositions[particle % numLegalPositions])
    ...
    def predict(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        particleDist, newLst = DiscreteDistribution(), [] #initialize dictionary and return list
        for particle in self.particles:
            if not particle in particleDist:
                particleDist[particle] = self.getPositionDistribution(gameState, particle) #sample from the new distribution
            newLst.append(particleDist[particle].sample()) #append this new sample
        self.particles = newLst #set self.particles to this return list
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        for particle in range(self.numParticles):
            self.particles.append(permutations[particle % lenPerm])
    ...
    def update(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for ghost in range(self.numGhosts):
                ...
                weight *= self.getObservationProb(observation[ghost], pacmanPosition, particle[ghost], jailPosition)
            beliefs[particle] += weight
    ...
    def predict(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                newLst.append(newPosDist.sample())
