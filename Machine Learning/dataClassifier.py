...
def enhancedFeatureExtractorDigit(datum):
    ...
    Perform Depth-first Search on the features to find how many regions are
    ...
    "*** YOUR CODE HERE ***"
    newFeatures = np.zeros(DIGIT_DATUM_WIDTH*DIGIT_DATUM_HEIGHT*2, int)
    edges = np.zeros(DIGIT_DATUM_WIDTH*DIGIT_DATUM_HEIGHT, int)
    insides = np.zeros(DIGIT_DATUM_WIDTH*DIGIT_DATUM_HEIGHT, int)
    n = DIGIT_DATUM_WIDTH*DIGIT_DATUM_HEIGHT
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if datum.getPixel(x, y) == 1:
                #insides
                newFeatures[DIGIT_DATUM_HEIGHT*x+y] = 1
                insides[DIGIT_DATUM_HEIGHT*x+y] = 1
            if datum.getPixel(x, y) == 2:
                #edges
                newFeatures[n+DIGIT_DATUM_HEIGHT*x+y] = 1
                edges[DIGIT_DATUM_HEIGHT*x+y] = 1
    ...
    #     if features[index]:
    #         total += 1
    # if total > len(features) / 2:
    #     lightDom = 1
    #do depth first search
    #add more stuff to end of features
    #count the number of clustered (4x4) pixels; if it's greater than some threshhold ()
    # sC = 2
    # sCV = 7
    # sCC = 38
    # mC = 3
    ...
    #         temp_count = 0
    #         if y >= sC and x >= sC and y <= DIGIT_DATUM_HEIGHT - sC and x <= DIGIT_DATUM_WIDTH - sC:
    #             # for small clusters
    #             for tempX in range(x - sC, x + sC):
    #                 for tempY in range(y - sC, y + sC):
    #                     temp_count += features[DIGIT_DATUM_HEIGHT*tempX + tempY]

    #             if temp_count > sCV:
    #                 sCount += 1
    #         temp_count = 0
    #         if y >= mC and x >= mC and y <= DIGIT_DATUM_HEIGHT - mC and x <= DIGIT_DATUM_WIDTH - mC:
    #             # for medium clusters
    #             for tempX in range(x - mC, x + mC):
    #                 for tempY in range(y - mC, y + mC):
    #                     temp_count += features[DIGIT_DATUM_HEIGHT*tempX + tempY]
    #             if temp_count > mCV:
    #                 mCount += 1
    #         temp_count = 0
    #         if y >= lC and x >= lC and y <= DIGIT_DATUM_HEIGHT - lC and x <= DIGIT_DATUM_WIDTH - lC:
    #             # for small clusters
    #             for tempX in range(x - lC, x + lC):
    #                 for tempY in range(y - lC, y + lC):
    #                     temp_count += features[DIGIT_DATUM_HEIGHT*tempX + tempY]
    #             if temp_count > lCV:
    #                 lCount += 1
    #         x += 2
    ...
    # print(mCount)
    # print(lCount)
    # markedFeatures = np.array(features)
    # print(markedFeatures)
# earnly
    # return newFeatures
    
    markedFeatures = np.array(features)
    markedInsides = np.array(insides)
    markedEdges = np.array(edges)
    # markedFeatures = np.array(features)
    count1 = 0
    count2 = 0
    count3 = 0
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if markedFeatures[DIGIT_DATUM_HEIGHT*x+y] == 0:
                count1 += 1
                dfs(markedFeatures, x, y)

    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if markedInsides[DIGIT_DATUM_HEIGHT*x+y] == 0:
                count2 += 1
                dfs(markedInsides, x, y)

    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if markedEdges[DIGIT_DATUM_HEIGHT*x+y] == 0:
                count3 += 1
                dfs(markedEdges, x, y)
    ...
    if count1 == 1:
        ...
        five = 1
    newLst = [one, two, three, four, five]
    one, two, three, four, five = 0, 0, 0, 0, 0
    ...
    #             count2 += 1
    #             dfs(markedInsides, x, y)
    # newLst = [sClusters, mClusters, lClusters, one, two, three]
    return np.append(features, newLst)
    # return np.append(newFeatures, newLst)

def dfs(features, x, y):
    "do depth-first search to find the regions of 'off' or '0' pixels and mark them to 'on' or '1' "
    features[DIGIT_DATUM_HEIGHT*x+y] = 1
    if x - 1 > 0:
        if features[DIGIT_DATUM_HEIGHT*(x-1)+y] == 0:
            dfs(features, x-1, y)
    if x + 1 < DIGIT_DATUM_WIDTH:
        if features[DIGIT_DATUM_HEIGHT*(x+1)+y] == 0:
            dfs(features, x+1, y)
    if y - 1 > 0:
        if features[DIGIT_DATUM_HEIGHT*x+(y-1)] == 0:
            dfs(features, x, y-1)
