...
def stochasticGradientDescentUpdate(datum, label, weights, alpha, der_loss_dw):
    ...
    """
    "*** YOUR CODE HERE ***"
    scalar = alpha * der_loss_dw(datum, label, weights)
    updatedWeights = np.subtract(weights, scalar)
...
def der_sigmoid_dx(x):
    ...
    "*** YOUR CODE HERE ***"
    e = math.e
    eToTheNegX = math.pow(e, -x)
    denominator = math.pow(eToTheNegX+1, 2)
    return eToTheNegX / denominator
    util.raiseNotDefined()
...
def softmax(x):
    ...
    array = []
    for xi in x:
        numerator = math.pow(math.e, xi)
        denominator = 0
        for xj in x:
            denominator += math.pow(math.e, xj)
        val = numerator / denominator
        array.append(val)
    return np.array(array)
        # e = math.pow(math.e, xi)
    util.raiseNotDefined()
...
class LinearRegression:
    ...
    def trainAnalytical(self, trainingData_x, trainingData_y):
        ...
        "*** YOUR CODE HERE ***"
        xTrans = np.dot(trainingData_x.T, trainingData_x)
        xTransInverse = np.linalg.inv(xTrans)
        xTransInverseXxTrans = np.dot(xTransInverse, trainingData_x.T)
        self.weights = np.dot(xTransInverseXxTrans, trainingData_y)

        # self.weights = np.linalg.inv(trainingData_x.T * trainingData_x) * trainingData_x.T * trainingData_y
...
class BinaryLinearClassifier:
    ...
    def hypothesis(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        dotPro = np.dot(x, self.weights)
        return sigmoid(dotPro)
        util.raiseNotDefined()
    ...
    def der_loss_dw(self, x, y_true, weights):
        ...
        "*** YOUR CODE HERE ***"
        hyp = self.hypothesis(x)
        quadDer = der_quadLoss_dx(hyp, y_true)
        sigDer = der_sigmoid_dx(np.dot(x, weights))
        dotDer = der_dot_dw(x, weights)
        return quadDer * sigDer * dotDer
        util.raiseNotDefined()
...
class MulticlassLinearClassifier:
    ...
    def hypothesis(self, x):
        ...
        for label in range(numClasses):
            dotPro = np.dot(x, self.weights[label])
            array.append(dotPro)
